...
@app.route('/tags/notes', methods=['GET'])
def get_note_tags():
    podcast_name = request.args.get('podcast', default='TMA', type=str)  # 'TMA' as default
    table_name_map = {
        'TMA': 'TMA_Note_Tags',
        'Balloon Party': 'Balloon_Note_Tags',
        'The Tim McKernan Show': 'TMShow_Note_Tags',
    }
    table_name = table_name_map.get(podcast_name, 'TMA_Note_Tags')  # Fallback to TMA tags
    return get_tags(table_name)

@app.route('/tags/titles', methods=['GET'])
def get_title_tags():
    podcast_name = request.args.get('podcast', default='TMA', type=str)
    table_name_map = {
        'TMA': 'TMA_Title_Tags',
        'Balloon Party': 'Balloon_Title_Tags',
        'The Tim McKernan Show': 'TMShow_Title_Tags',
    }
    table_name = table_name_map.get(podcast_name, 'TMA_Title_Tags')
    return get_tags(table_name)
...
def get_tags(table_name):
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            cursor = conn.cursor()
            cursor.execute(f"SELECT tag, count FROM {table_name} ORDER BY count DESC")
            tags = cursor.fetchall()
            tags_data = [dict(tag=row['tag'], count=row['count']) for row in tags]
