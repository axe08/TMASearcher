...
LOG_FILE_PATH = '/home/axe08admin/Web_App/tmshow_podcast_scraping.log'
...
def setup_database():
    ...
    cursor = conn.cursor()
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS TMShow (
...
def insert_episode(title, date, url, show_notes):
    ...
    try:
        cursor.execute('''
            INSERT INTO TMShow (TITLE, DATE, URL, SHOW_NOTES)
            VALUES (?, ?, ?, ?)
...
def scrape_latest_podcasts(pages_to_scrape):
    ...
    delay = 3  # Delay when searching multiple pages
...
# Run the scrape function for this many pages
